...
`backend-chat`, `backend-voice`, `database-chat`, `database-voice`. The slug is
truncated to make room, never the suffix. The slug is the project's **effective**
slug (`projectSlug`): its stored `docsSlug`, or `slugify(name)` when that is null.
Comparing raw `docsSlug` columns instead of the effective slug is exactly the bug
that let `UBS-Doc` and a NULL-slugged `UBS Doc` fight over the same ten channels
(fixed as final-wave A6/D4 — see "Duplicate slugs" below).
...
covers ids stored on **project** rows (category + ten section channels), not task
...
- **50 channels per category** → `CATEGORY_SOFT_CAP = 49` (`bot/src/constants.js`).
  Ten are the section itself; task channels beyond the cap fall back to the global
...
- **A category's overwrites are copied onto a channel only AT CREATION, never
  ...
  while its role was refused gets ten channels that copy the deny-only category and
...
(above), a section built while the role was refused has ten channels that stay
deny-only forever unless something explicitly repairs them. `planChannels` now
checks each of the ten channels for the gate role's overwrite
...
ten channel names forever, dragging the same channels back and forth between
...
whose id is a project's `discordCategoryId`, its ten `claimedSectionIds`, and
